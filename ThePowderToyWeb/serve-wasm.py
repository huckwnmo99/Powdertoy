...
CERT_CN    = 'serve-wasm.py self-signed certificate for powder_kokr.js'
CERT_TTL   = 30 * 24 * 60 * 60

print(f'generating index for powder_kokr.js')
with open(HTTP_INDEX, 'wt') as f:
	f.write(f'''
<!DOCTYPE html>
<html lang="en">
	<body>
		<p>Remember, this is being served locally, so it will not be able to connect to <a href="https://tptserv.starcatcher.us">https://tptserv.starcatcher.us</a>. To enable this, serve the following files there:</p>
		<ul>
			<li>powder_kokr.js</li>
			<li>powder_kokr.wasm</li>
			<li>powder_kokr.wasm.map for debugging purposes</li>
		</ul>
		...
		<script type='text/javascript'>
			(() => {{
				...
				window.create_powder_kokr_loader = () => {{
					if (promise === undefined) {{
						promise = new Promise((resolve, reject) => {{
							const script = document.createElement('script');
							script.onload = () => {{
								resolve(window.create_powder_kokr);
							}};
							document.head.appendChild(script);
							script.src = 'powder_kokr.js';
						}});
					}}
					return promise;
				}};
			...
			(() => {{
				...
				create_powder_kokr_loader().then(create_powder_kokr => create_powder_kokr({{
					canvas: (() => {{
